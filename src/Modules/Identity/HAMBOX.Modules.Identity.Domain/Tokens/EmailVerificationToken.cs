...
public sealed class EmailVerificationToken : Entity
{
    ...
    /// <summary>
    /// Gets the SHA-256 hash of the verification token value. The plaintext is never persisted — it
    /// exists only for the moment it's generated and emailed to the user.
    /// </summary>
    ...
    /// <summary>
    /// Creates a new email verification token, persisting only the SHA-256 hash of the plaintext value.
    /// </summary>
    ...
    public static EmailVerificationToken Create(Guid userId, string plaintextToken, DateTimeOffset expiresOnUtc)
    {
        ...
        return new EmailVerificationToken(Guid.NewGuid(), userId, ComputeHash(plaintextToken), expiresOnUtc);
    }

    /// <summary>
    /// Computes the stored lookup hash for a plaintext email verification token.
    /// </summary>
    /// <param name="plaintextToken">The plaintext token supplied by the caller (e.g. from the verification link).</param>
