...
internal sealed class BanUserCommandHandler(
    ...
    IBackgroundJobScheduler jobScheduler) : IRequestHandler<BanUserCommand, Result>
{
    public async Task<Result> Handle(BanUserCommand request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            user.Ban(request.Reason, request.Notes, actorUserId == Guid.Empty ? null : actorUserId);
        }
        ...
        await securityEventLogger.LogAsync(
            SecurityEventType.ManualBan,
            SecurityEventSeverity.Critical,
            $"User {user.Email} was permanently banned: {request.Reason}",
