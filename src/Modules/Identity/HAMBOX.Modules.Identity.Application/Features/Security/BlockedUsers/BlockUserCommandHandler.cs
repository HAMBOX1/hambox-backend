...
internal sealed class BlockUserCommandHandler(
    ...
    IBackgroundJobScheduler jobScheduler) : IRequestHandler<BlockUserCommand, Result>
{
    public async Task<Result> Handle(BlockUserCommand request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            user.Block(request.Reason, request.Notes, actorUserId == Guid.Empty ? null : actorUserId, request.ExpiresOnUtc);
        }
        ...
        await securityEventLogger.LogAsync(
            SecurityEventType.AdminBlock,
            SecurityEventSeverity.High,
            $"User {user.Email} was {(request.ExpiresOnUtc.HasValue ? "temporarily" : "indefinitely")} blocked: {request.Reason}",
